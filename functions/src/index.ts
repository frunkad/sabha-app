    ...
    return db.collection('loksabha-ministers').where('constituency','==',constituency).limit(1).get()
        .then((snapshot) => {
            ...
            return snapshot.forEach(doc => {
                ...
                const data = doc.data();
                conv.close(`${data['name']} is the MP from ${data['constituency']}, ${data['state']}.
...
// Updations 
async function updateLokSabha(fileName) {
    ...
    const res = await bucket.file('assets/'+fileName).download({
        destination: tempFilePath,
    });

    const lsMP_ref = db.collection('loksabha-ministers');
    const workbook = await XLSX.readFile(tempFilePath);
    const sheet_name_list = workbook.SheetNames;
    const xlData = await XLSX.utils.sheet_to_json(workbook.Sheets[sheet_name_list[0]]);
    for (let i = 0; i < xlData.length; i++) {
        const mp = xlData[i];
        let newRef = await lsMP_ref.doc();
        if(mp['Constituency'] !== '' && mp['End of term'] === "In office")
        {
            try{
                const rfp = await newRef.set({
                    name: mp['MP name'],
...
export const redoInit = functions.firestore.document('server/current').onUpdate((change,context) =>{
    const dataOld = change.before.data();
    const data = change.after.data();
    const redo: boolean = data['count'] !== dataOld['count']; // force
